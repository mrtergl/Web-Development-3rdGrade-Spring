...
<aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-left ms-3"
    ...
    <div class="sidenav-header">
        ...
        <a class="navbar-brand m-0" href="/yoneticiekran/{{$id}}">
            <img src="../assets/img/logo-ct.png" class="navbar-brand-img h-100" alt="...">
            <span class="ms-1 font-weight-bold">Sistem Yöneticisi</span>
        </a>
    </div>
        ...
        <ul class="navbar-nav">
            ...
            <li class="nav-item pb-2">
                <a class="nav-link {{ Route::currentRouteName() == 'user-management' ? 'active' : '' }}"
                    ...
                    <span class="nav-link-text ms-1">Danışman Ekle</span>
                </a>
                <a class="nav-link {{ Route::currentRouteName() == 'user-management' ? 'active' : '' }}"
                    href="/ogrenciekle/{{$id}}">
                    <div
                ...
                <a class="nav-link {{ Route::currentRouteName() == 'user-management' ? 'active' : '' }}"
                    href="/danismanlarigoruntule">
                    <div
                        class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
                        <i style="font-size: 1rem;" class="fas fa-lg fa-list-ul ps-2 pe-2 text-center
                        {{ in_array(request()->route()->getName(),['user-management']) ? 'text-white' : 'text-dark' }}"></i>
                    </div>
                    <span class="nav-link-text ms-1">Danışmanlar</span>
                </a>
                <a class="nav-link {{ Route::currentRouteName() == 'user-management' ? 'active' : '' }}"
                    href="/ogrencigoruntule/{{$id}}">
                    <div
                        class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
                        <i style="font-size: 1rem;" class="fas fa-lg fa-list-ul ps-2 pe-2 text-center
                        {{ in_array(request()->route()->getName(),['user-management']) ? 'text-white' : 'text-dark' }}"></i>
            ...
            <div class="container-fluid py-4">
                <div class="card">
                    <div class="card-header pb-0 px-3">
                        <h6 class="mb-0">{{ __('Profil Bilgileri') }}</h6>
                    </div>
                    <div class="card-body pt-4 p-3">
                        ...
                        <form action="{{url('yonetici-guncelle/'.$item->yonetici_id) }}" method="POST" role="form text-left">
                            @csrf
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    <label for="">Ad</label>
                                    <input type="text" class="form-control" name="ad" value="{{$item->ad}}" style="border-bottom: 1px solid red;" required>
                                </div>
    
                                <div class="col-md-6 mb-3">
                                    <label for="">Soyad</label>
                                    <input type="text" class="form-control" name="soyad" value="{{$item->soyad}}"  style="border-bottom: 1px solid red;" required>
                                </div>
                                <div class="col-md-6">
                                    <div class="col-md-6 mb-3">
                                        <label for="">Email</label>
                                        <input type="email" class="form-control" name="eposta" value="{{$item->eposta}}"  style="border-bottom: 1px solid red;" required>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="col-md-6 mb-3">
                                        <label for="">Şifre</label>
                                        <input type="text" class="form-control" name="sifre" value="{{$item->sifre}}"  style="border-bottom: 1px solid red;" required>
                                    </div>
                                </div>
                            </div>
                            <div class="d-flex justify-content-end">
                                <button type="submit" class="btn bg-gradient-dark btn-md mt-4 mb-4">{{ 'Güncelle' }}</button>
                            </div>
                        </form>
